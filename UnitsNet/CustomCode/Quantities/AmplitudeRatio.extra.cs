...
namespace UnitsNet
{
    public partial struct AmplitudeRatio
    {
        ...
        /// </exception>
        public AmplitudeRatio(ElectricPotential voltage, int significantDigits = 15)
            : this()
        {
            if (voltage.Volts <= 0)
            {
            ...
            _value = QuantityValue.FromDoubleRounded(20 * Math.Log10(voltage.Volts.ToDouble() / 1), significantDigits);
        ...
        ///     </example>
        /// </remarks>
        public ElectricPotential ToElectricPotential(int significantDigits = 15)
        {
            // E(V) = 1V * 10^(E(dBV)/20)
            return ElectricPotential.FromVolts(QuantityValue.FromDoubleRounded(Math.Pow(10, DecibelVolts.ToDouble() / 20), significantDigits));
        }
        ...
        public PowerRatio ToPowerRatio(ElectricResistance impedance, int significantDigits = 15)
        {
            // P(dBW) = E(dBV) - 10*log10(Z(Ω)/1)
            return PowerRatio.FromDecibelWatts(DecibelVolts - QuantityValue.FromDoubleRounded(10 * Math.Log10(impedance.Ohms.ToDouble() / 1), significantDigits));
        }
        ...
        ///     Thrown when the number of significant digits is less than 1 or greater than 17.
        /// </exception>
        public static AmplitudeRatio FromElectricPotential(ElectricPotential voltage, int significantDigits = 15)
        {
